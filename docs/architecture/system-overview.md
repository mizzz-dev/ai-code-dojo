...
最終更新: 2026-07-24（Issue #111 queue contract / HTTP adapter分離を反映）
...
6. Workerが共通message contractでrequestを検証する。
7. WorkerがDB上のsubmissionを条件付きclaimする。
8. heartbeat有効時はWorkerがprocessing leaseを定期延長する。
9. stale recovery有効時はWorkerが起動時・定期的に期限切れleaseを走査する。
10. WorkerがRunnerでvisible/hidden testsを実行する。
11. Workerがexpected attempt/keyによるfenced updateとcompletion guardで結果を保存する。
12. Webがsubmission結果をポーリング表示する。
...
- transport observabilityとapplication retry backoff seamを次に実装する。
...
- Issue #111ではDB schema / migrationを変更しない。
...
- queue message / logsへ提出コード本文・hidden tests実データ・secretを記録しない。
