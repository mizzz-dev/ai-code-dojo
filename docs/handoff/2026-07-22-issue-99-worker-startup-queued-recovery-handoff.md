...
## Summary
Worker起動時にDB上の `queued` submissionを回収し、SQLiteの条件付きUPDATEで `queued -> running` をclaimする実装を追加した。PR #100はReady for reviewへ切り替え済みで、integration testを含むGitHub Actions品質ゲートは全て成功している。
...
## Remaining Tasks
1. PR #100をレビューする。
2. merge後にIssue #99とLinear MIZ-19を完了へ更新する。
3. merge後にhead branchを削除する。
4. stale `running` recovery設計を後続Issueへ分離する。
